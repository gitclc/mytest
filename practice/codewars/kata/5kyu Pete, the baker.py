...
# which takes the recipe (object) and the available ingredients (also an object) and returns
...
def cakes(recipe, available):
    n = None
    for i in recipe:
        use = available.get(i, 0) // recipe[i]
        if not use:
            return 0
        if n is None:
            n = use
...
if __name__ == '__main__':
    a = cakes({'flour': 300, 'sugar': 150, 'milk': 100},
